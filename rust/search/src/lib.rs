...
pub fn fuzzy_score<S: ToString>(query: &S, data: &S) -> u32 {
    ...
    for &start in &start_indices {
        ...
        // 遍历目标字符串剩余部分
        for (i, item) in d_chars.iter().enumerate().skip(start + 1) {
            if q_index >= q_chars.len() {
                break; // 已匹配所有查询字符
            }

            if item == &q_chars[q_index] {
                // 基础分
                score += 1;
 ...
 * 返回数据列表中的对应得分
 */
pub fn fuzzy_search_score<S: ToString>(query: &S, data: &[S]) -> Vec<u32> {
 ...
 * 模糊搜索并返回排序后的字符
 */
pub fn fuzzy_search<S: ToString>(query: &S, data: &[S]) -> Vec<String> {
